...
│   └── modules/
│       └── FindODB.cmake           # 手动创建：用于 find_package 查找 ODB 编译器及库

├── proto/                          # 手动创建：放置 .proto 文件（服务和数据结构定义）
│   ├── service.proto               # 手动创建：定义 gRPC 服务接口
│   ├── model.proto                 # 手动创建：定义消息结构、数据库模型等
│   └── CMakeLists.txt              # 手动创建：集成 protoc & grpc 插件自动生成代码
...
│   └── mygrpc/
│       ├── server.hpp              # 手动创建：gRPC 服务类声明（可能引用 .proto 定义）
│       ├── db.hpp                  # 手动创建：数据库操作接口或桥接定义
│       └── config.hpp              # 手动创建：配置加载相关接口定义

├── src/                            # 手动创建：主业务代码目录
│   ├── main.cpp                    # 手动创建：程序入口（创建 gRPC 服务器、加载配置等）
│   ├── grpc/
│   │   └── service_impl.cpp        # 手动创建：gRPC 服务实现（继承自动生成的 stub）
│   ├── db/
│   │   ├── user.hpp                # 手动创建：业务模型类 + ODB 映射注解
│   │   ├── user-odb.hxx            # 自动生成：由 ODB 编译器根据 user.hpp 生成（不要手动修改）
│   │   └── user.cpp                # 手动创建：实现数据库操作（插入、查询、更新等）
│   ├── utils/
│   │   ├── logger.cpp              # 手动创建：封装日志系统（如 spdlog）
│   │   └── logger.hpp              # 手动创建：日志模块头文件
│   └── config/
│       └── config_loader.cpp       # 手动创建：配置解析实现（通常使用 yaml-cpp）

├── config/                         # 手动创建：项目运行时配置文件目录
│   ├── db.yaml                     # 手动创建：数据库连接配置（DSN、池大小等）
│   └── server.yaml                 # 手动创建：服务配置（端口号、线程数、日志级别）

├── scripts/                        # 手动创建：常用命令脚本（构建、运行、协议编译等）
│   ├── build.sh                    # 手动创建：一键构建脚本
│   ├── run_dev.sh                  # 手动创建：开发环境启动服务
│   └── generate_proto.sh           # 手动创建：调用 protoc 和 ODB 编译器生成代码

├── test/                           # 手动创建：单元测试代码（建议使用 Google Test）
│   ├── test_db.cpp                 # 手动创建：数据库相关功能测试
│   └── test_grpc.cpp               # 手动创建：gRPC 接口测试

├── benchmark/                      # 可选：性能测试模块（推荐用于关键路径分析）
│   ├── CMakeLists.txt              # 手动创建：benchmark 模块的构建文件
│   └── bench_insert_vs_batch.cpp   # 手动创建：示例性能测试代码（可测试批量插入效率等）

├── docs/                           # 可选：文档目录，适用于团队协作或开源项目
│   ├── architecture.md             # 手动创建：项目架构图、模块关系说明
│   ├── database-design.md          # 手动创建：数据模型及 ODB 映射说明
│   ├── grpc-api.md                 # 手动或自动生成：gRPC 接口说明文档
│   └── deployment.md               # 手动创建：部署说明，运行环境依赖、命令示例等

├── third_party/                    # 可选：手动引入的第三方库（建议用 vcpkg/conan 管理）
│   └── ...                         # 可用于 header-only 库，如 fmt, nlohmann/json 等

├── docker/                         # 可选：用于容器化部署
│   └── Dockerfile                  # 手动创建：构建服务镜像的 Dockerfile
